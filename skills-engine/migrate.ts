...
import { BASE_DIR, CUSTOM_DIR, JARV1S_DIR } from './constants.js';
import { initJarv1sDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initJarv1sDir();
  console.log('Skills system initialized. .jarv1s/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .jarv1s/custom/migration.patch',
      );
    } else {
