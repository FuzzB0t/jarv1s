...
import { BASE_DIR, JARV1S_DIR } from './constants.js';
...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, JARV1S_DIR, 'state.yaml');
    ...
    try {
      ...
      // Save combined patch
      const patchPath = path.join(projectRoot, JARV1S_DIR, 'combined.patch');
      ...
      if (newBasePath) {
        ...
        for (const relPath of trackedFiles) {
          ...
          const tmpSaved = path.join(
            os.tmpdir(),
            `jarv1s-rebase-${crypto.randomUUID()}-${path.basename(relPath)}`,
          );
