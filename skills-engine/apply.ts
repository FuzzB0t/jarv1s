...
import { JARV1S_DIR } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, JARV1S_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `jarv1s-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
