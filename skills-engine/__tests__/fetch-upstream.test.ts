...
describe('fetch-upstream.sh', () => {
  ...
  beforeEach(() => {
    ...
    upstreamBareDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'jarv1s-upstream-'),
    );
    ...
    const seedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jarv1s-seed-'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'jarv1s', version: '2.0.0' }),
    );
    ...
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jarv1s-project-'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'jarv1s', version: '1.0.0' }),
    );
  ...
  it('uses existing upstream remote', () => {
    ...
    expect(status.TEMP_DIR).toMatch(/^\/tmp\/jarv1s-update-/);
  ...
  it('uses origin when it points to qwibitai/jarv1s', () => {
    // Set origin to a URL containing qwibitai/jarv1s
    execSync(`git remote add origin https://github.com/qwibitai/jarv1s.git`, {
      cwd: projectDir,
      stdio: 'pipe',
    });
  ...
  it('adds upstream remote when none exists', { timeout: 15_000 }, () => {
    ...
    expect(remotes).toContain('qwibitai/jarv1s');
  });
