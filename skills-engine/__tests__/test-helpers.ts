...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'jarv1s-test-'));
}

export function setupJarv1sDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.jarv1s', 'base', 'src'), {
    recursive: true,
  });
  fs.mkdirSync(path.join(tmpDir, '.jarv1s', 'backup'), { recursive: true });
}

export function writeState(tmpDir: string, state: any): void {
  const statePath = path.join(tmpDir, '.jarv1s', 'state.yaml');
