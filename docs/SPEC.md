...
│  │    • mcp__jarv1s__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── jarv1s.log               # Host stdout
│   └── jarv1s.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.jarv1s.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'jarv1s-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your jarv1s installation
...
Jarv1s uses a hierarchical memory system based on CLAUDE.md files.
...
Jarv1s has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__jarv1s__schedule_task]
        {
...
The `jarv1s` MCP server is created dynamically per agent call with the current group's context.
...
Jarv1s runs as a single macOS launchd service.

### Startup Sequence

When Jarv1s starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Jarv1s containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/jarv1s.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/jarv1s.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.jarv1s.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.jarv1s.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.jarv1s.plist

# Check status
launchctl list | grep jarv1s

# View logs
tail -f logs/jarv1s.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep jarv1s` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Jarv1s auto-starts container runtime but may fail |
...
- `logs/jarv1s.log` - stdout
- `logs/jarv1s.error.log` - stderr
