...
<dict>
    ...
    <string>${projectRoot}/logs/jarv1s.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/jarv1s.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.jarv1s');
  } catch {
...
/**
 * Kill any orphaned jarv1s node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned jarv1s processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/jarv1s.service';
    ...
    unitPath = path.join(unitDir, 'jarv1s.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=Jarv1s Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/jarv1s.log
StandardError=append:${projectRoot}/logs/jarv1s.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned jarv1s processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable jarv1s`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start jarv1s`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active jarv1s`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-jarv1s.sh');
  const pidFile = path.join(projectRoot, 'jarv1s.pid');

  const lines = [
    '#!/bin/bash',
    '# start-jarv1s.sh — Start Jarv1s without systemd',
    ...
    '    echo "Stopping existing Jarv1s (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting Jarv1s..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/jarv1s.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/jarv1s.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Jarv1s started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/jarv1s.log"`,
  ];
