---
name: add-gmail
description: Add Gmail integration to Jarv1s. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail support to Jarv1s — either as a tool (read, send, search, draft) or as a full channel that polls the inbox.

## Phase 1: Pre-flight

### Check if already applied

Read `.jarv1s/state.yaml`. If `gmail` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
If `.jarv1s/` directory doesn't exist yet:
...
Add `gmail` to `.jarv1s/state.yaml` under `applied_skills` with `mode: tool-only`.
...
- Records the application in `.jarv1s/state.yaml`
...
>    - Application type: **Desktop app**, name: anything (e.g., "Jarv1s Gmail")
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.jarv1s  # macOS
# Linux: systemctl --user restart jarv1s
```
...
Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/jarv1s.log | grep -iE "(gmail|email)"`.
...
3. Remove `gmail` from `.jarv1s/state.yaml`
4. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
5. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.jarv1s` (macOS) or `systemctl --user restart jarv1s` (Linux)
...
7. Remove `gmail` from `.jarv1s/state.yaml`
8. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
9. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.jarv1s` (macOS) or `systemctl --user restart jarv1s` (Linux)
