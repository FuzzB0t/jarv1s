...
- Added: `gmail` MCP server alongside the existing `jarv1s` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `jarv1s` MCP server configuration is unchanged
...
## Must-keep
- The `jarv1s` MCP server with its environment variables
