...
Adds Parallel AI MCP integration to Jarv1s for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Jarv1s scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Jarv1s already set up and running
...
Add Parallel AI MCP servers after the jarv1s server:
```typescript
const mcpServers: Record<string, any> = {
  jarv1s: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__jarv1s__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__jarv1s__send_message
   4. Use mcp__jarv1s__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo jarv1s-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep jarv1s  # macOS
...
- Check task runs: `tail -f logs/jarv1s.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.jarv1s` (macOS) or `systemctl --user restart jarv1s` (Linux)
