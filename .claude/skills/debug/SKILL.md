...
|-----|----------|---------|
| **Main app logs** | `logs/jarv1s.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/jarv1s.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash jarv1s-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash jarv1s-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash jarv1s-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash jarv1s-agent:latest
```
...
# Also clear the session ID from Jarv1s's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/jarv1s.log | tail -5
...
```bash
echo "=== Checking Jarv1s Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo jarv1s-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/jarv1s.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
