...
This skill adds Telegram support to Jarv1s using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.jarv1s/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep jarv1s` (macOS) or `systemctl --user status jarv1s` (Linux)
...
- Check bot is started: `tail -f logs/jarv1s.log`
...
# Linux:
# systemctl --user stop jarv1s
# npm run dev
# systemctl --user start jarv1s
```
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.jarv1s` (macOS) or `npm run build && systemctl --user restart jarv1s` (Linux)
