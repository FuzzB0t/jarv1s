---
name: x-integration
description: X (Twitter) integration for Jarv1s. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** Jarv1s v1.0.0. Directory structure may change in future versions.
...
1. **Jarv1s is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep jarv1s (macOS) or systemctl --user status jarv1s (Linux)
```
...
| `JARV1S_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/jarv1s.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into Jarv1s, make the following modifications:
...
All paths below are relative to project root (`JARV1S_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep jarv1s  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status jarv1s
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/jarv1s.log | tail -20

# Script errors
grep -i "error\|failed" logs/jarv1s.log | tail -20
```
...
docker run jarv1s-agent ls -la /app/src/skills/
```
