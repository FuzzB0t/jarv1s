# Slack App Setup for Jarv1s

Step-by-step guide to creating and configuring a Slack app for use with Jarv1s.

## Prerequisites

- A Slack workspace where you have admin permissions (or permission to install apps)
- Your Jarv1s instance with the `/add-slack` skill applied
...
3. When prompted for a token name, enter something like `jarv1s`
4. Click **Generate**
...
You need the Slack channel ID to register it with Jarv1s.
...
The Jarv1s JID format is `slack:` followed by the channel ID, e.g., `slack:C0123456789`.
