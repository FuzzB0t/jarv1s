...
This skill adds Slack support to Jarv1s using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.jarv1s/state.yaml`. If `slack` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
>
> The JID format for Jarv1s is: `slack:C0123456789`
...
4. Service is running: `launchctl list | grep jarv1s`
...
6. Restart: `launchctl kickstart -k gui/$(id -u)/com.jarv1s`
