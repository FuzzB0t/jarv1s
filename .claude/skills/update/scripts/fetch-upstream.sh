...
# Fetch upstream Jarv1s and extract to a temp directory.
...
if git remote get-url upstream &>/dev/null; then
  ...
  if echo "$ORIGIN_URL" | grep -q "qwibitai/jarv1s"; then
    REMOTE="origin"
  fi
fi

if [ -z "$REMOTE" ]; then
  echo "No upstream remote found. Adding upstream → https://github.com/qwibitai/jarv1s.git"
  git remote add upstream https://github.com/qwibitai/jarv1s.git
  REMOTE="upstream"
fi
...
TEMP_DIR=$(mktemp -d /tmp/jarv1s-update-XXXX)
