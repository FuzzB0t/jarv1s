---
name: update
description: "Update Jarv1s from upstream. Fetches latest changes, merges with your customizations and skills, runs migrations. Triggers on \"update\", \"pull upstream\", \"sync with upstream\", \"get latest changes\"."
---
...
```bash
test -d .jarv1s && echo "INITIALIZED" || echo "NOT_INITIALIZED"
```

**If NOT_INITIALIZED:** Run `initSkillsSystem()` first:

```bash
npx tsx -e "import { initJarv1sDir } from './skills-engine/init.js'; initJarv1sDir();"
```
...
**No upstream remote:** The fetch script auto-adds `upstream` pointing to `https://github.com/qwibitai/jarv1s.git`. If the user forked from a different URL, they should set the remote manually: `git remote add upstream <url>`.
...
**Rollback:** If something goes wrong after applying but before cleanup, the backup is still in `.jarv1s/backup/`. Run:
```bash
