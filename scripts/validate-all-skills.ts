 ...
 *   2. Initialize .jarv1s/ (snapshot current source as base)
...
function resetWorkingTree(): void {
  ...
  if (fs.existsSync('.jarv1s')) {
    fs.rmSync('.jarv1s', { recursive: true, force: true });
  }
}

function initJarv1s(): void {
  execSync(
    'npx tsx -e "import { initJarv1sDir } from \'./skills-engine/index\'; initJarv1sDir();"',
